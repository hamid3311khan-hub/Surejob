...
def candidate_login():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('surejob.db')
        c = conn.cursor()
        c.execute('SELECT id, name, password FROM candidates WHERE email =?', (data['email'],))
        user = c.fetchone()
        conn.close()
...
def company_register():
    if request.method == 'POST':
        ...
        c = conn.cursor()
        try:
            c.execute('''INSERT INTO companies (company_name, email, password, phone, industry, address)
                        VALUES (?,?,?,?,?,?)''',
                     (data['company_name'], data['email'], hashed_pw,
                      data.get('phone'), data.get('industry'), data.get('address')))
            conn.commit()
            conn.close()
            return jsonify({'success': True})
        except sqlite3.IntegrityError:
...
def post_job():
    ...
    if request.method == 'POST':
        data = request.form
        company_id = session['company_id']
        salary = f"₹{data.get('salary_min', '')}-{data.get('salary_max', '')} LPA"
        if data.get('salary_not_disclosed'):
...
def job_applications(job_id):
    ...
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute('''SELECT a.*, c.name, c.email, c.phone, j.title as job_title
                 FROM applications a 
